...
def profile_html_report(profile: Any, *, title: str | None = None, audience: str = "general") -> str:
    ...
    subtitle = title or f"EchoWave report 鈥?{card['domain']}"
    body = f"""
  <section class='hero'>
    <div class='card'>
      ...
      <p class='lead'>Paste a time-series dataset and get a explainable similarity before you choose a model.</p>
      <div class='facts'>
      ...
      <ul>{actions}</ul>
      <div class='signature'>Generated by EchoWave for an audience of <strong>{escape(audience)}</strong>. Use this as a pre-model audit and handoff artifact, not as a modelling guarantee.</div>
    </div>
...
def similarity_html_report(report: Any, *, title: str | None = None, left_series: Any | None = None, right_series: Any | None = None, rolling_windows: Iterable[Mapping[str, Any]] | None = None) -> str:
    ...
    title_text = title or f"EchoWave similarity report 鈥?{report.left_name} vs {report.right_name}"
  ...
  <section class='hero'>
    <div class='card'>
      ...
      <p class='muted'>{escape(report.left_name)} vs {escape(report.right_name)} 路 {escape(report.qualitative_label)}</p>
...
def _social_card_shell(*, title: str, subtitle: str, bullets: list[str], accent: str, width: int = 1200, height: int = 630) -> str:
    bullets_svg = "".join(
        f"<text x='70' y='{225 + 58*i}' font-size='30' fill='#17324d'>鈥?{escape(item)}</text>" for i, item in enumerate(bullets[:4])
    )
    return _svg_wrap(
        ...
        width=width,
        height=height,
        title=f"EchoWave social card 鈥?{title}",
    )
...
def profile_social_card_svg(profile: Any, *, title: str | None = None) -> str:
    ...
    return _social_card_shell(
        ...
        subtitle=f"{card['domain']} 路 reliability {card['dataset_facts']['reliability']['score']:.2f}",
        bullets=[f"Top axis: {b}" for b in bullets] + ["Report before you choose a model"],
        accent="#0b6cff",
    )
